"""REST API for Training Jobs — thin router, delegates to repositories."""
...
def list_training_jobs(
    ...
    rows, total = job_repo.list_all(status=status, submitted_by=submitted_by, limit=limit, offset=offset)
...
def create_training_job(payload: TrainingJobCreate):
    return job_repo.create(
        job_id=payload.id, dataset_version=payload.dataset_version,
        train_config=payload.train_config, backend=payload.backend,
