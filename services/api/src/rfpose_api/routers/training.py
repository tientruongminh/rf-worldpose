    ...
    with connect() as conn, conn.cursor() as cur:
        cur.execute(
            ...
            (payload.id, payload.dataset_version, payload.train_config,
             payload.backend, payload.submitted_by or "api"),
        )
        return cur.fetchone()
...
def mark_submitted(job_id: str, slurm_job_id: str):
    with connect() as conn, conn.cursor() as cur:
        cur.execute(
            """UPDATE training_jobs SET status='submitted', slurm_job_id=%s,
               submitted_at=now(), updated_at=now() WHERE id=%s RETURNING *""",
            (slurm_job_id, job_id),
        )
        return cur.fetchone()
